...
public class MovementSpeed : CommandRoot
{
    protected override void BuildTree(Console con)
    {
        Branch("walkSpeed", () =>
        {
            Leaf("get", delegate
            {
                con.PrintLine($"Current walk speed: {NewMovement.Instance.walkSpeed}");
            });
        ...
        Branch("jumpPower", () =>
        {
            Leaf("get", delegate
            {
                con.PrintLine($"Current jump power: {NewMovement.Instance.jumpPower}");
            });
        ...
        Branch("airAcceleration", () =>
        {
            Leaf("get", delegate
            {
                con.PrintLine($"Current air acceleration: {NewMovement.Instance.airAcceleration}");
            });
        ...
        Branch("wallJumpPower", () =>
        {
            Leaf("get", delegate
            {
                con.PrintLine($"Current wall jump power: {NewMovement.Instance.wallJumpPower}");
            });
            Leaf("set", delegate(float newPower)
            {
